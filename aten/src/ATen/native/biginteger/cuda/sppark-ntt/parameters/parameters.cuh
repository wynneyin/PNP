...
void NTTParameters(bool inverse, fr_t* data_ptr, fr_t* local_params)
{
    ...
    cudaMemcpyAsync(local_params, NTTHyperParam<fr_t>::inverse_roots_of_unity.data(), 
                    inverse_sz * sizeof(fr_t), cudaMemcpyHostToDevice);
    C10_CUDA_KERNEL_LAUNCH_CHECK();

    cudaMemcpyAsync(local_params + inverse_sz, NTTHyperParam<fr_t>::forward_roots_of_unity.data(), 
                    inverse_sz * sizeof(fr_t), cudaMemcpyHostToDevice);
    C10_CUDA_KERNEL_LAUNCH_CHECK();
    ...
    cudaMemcpyAsync(local_params + 2 * inverse_sz, NTTHyperParam<fr_t>::group_gen_inverse.data(), 
                        sizeof(fr_t), cudaMemcpyHostToDevice);
    C10_CUDA_KERNEL_LAUNCH_CHECK();

    cudaMemcpyAsync(local_params + 2 * inverse_sz + 1, NTTHyperParam<fr_t>::group_gen.data(), 
                        sizeof(fr_t), cudaMemcpyHostToDevice);
    C10_CUDA_KERNEL_LAUNCH_CHECK();


    generate_partial_twiddles<<<WINDOW_SIZE/32, 32, 0>>>
        (data_ptr + partial_sz, inverse ? (local_params + 2 * inverse_sz)
                                            : (local_params + 2 * inverse_sz + 1));
    C10_CUDA_KERNEL_LAUNCH_CHECK();


    generate_all_twiddles<<<blob_sz/32, 32, 0>>>(data_ptr + 2 * partial_sz,
                                                        roots+6,
                                                        roots+7,
                                                        roots+8,
                                                        roots+9,
    ...
    generate_radixX_twiddles_X<<<16, 64, 0>>>(data_ptr + 2 * partial_sz + blob_sz, 64, roots+12);
    generate_radixX_twiddles_X<<<16, 64, 0>>>(data_ptr + 2 * partial_sz + blob_sz + 64*64, 4096, roots+18);
    generate_radixX_twiddles_X<<<16, 128, 0>>>(data_ptr + 2 * partial_sz + blob_sz + 64*64 + 4096*64, 128, roots+14);
    generate_radixX_twiddles_X<<<16, 256, 0>>>(data_ptr + 2 * partial_sz + blob_sz + 64*64 + 4096*64 + 128*128, 256, roots+16);
    generate_radixX_twiddles_X<<<16, 512, 0>>>(data_ptr + 2 * partial_sz + blob_sz + 64*64 + 4096*64 + 128*128 + 256*256, 512, roots+18);
