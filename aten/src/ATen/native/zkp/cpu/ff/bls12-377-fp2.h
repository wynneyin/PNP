...
class fp2_t : public fp_mont {
 private:
  ...
  static inline fp_mont mul_by_5(const fp_mont& a) {
    fp_mont ret;
    asm("shl.b32 %0, %1, 2; add.cc.u32 %0, %0, %1;" : "=r"(ret[0]) : "r"(a[0]));
    for (size_t i = 1; i < a.len(); i++) {
 ...
 public:
  ...
  class mem_t {
   ...
   public:
    inline operator fp2_t() const {
      return x[threadIdx.x & 1];
    }
    inline void zero() {
      x[threadIdx.x & 1].zero();
    }
    inline void to() {
      x[threadIdx.x & 1].to();
    }
    inline void from() {
      x[threadIdx.x & 1].from();
    }
  ...
  inline fp2_t(const mem_t* p) {
    *this = p->x[threadIdx.x & 1];
  }
  inline void store(mem_t* p) const {
    p->x[threadIdx.x & 1] = *this;
  }
...
class fp2_t {
 ...
 public:
  ...
  inline fp2_t& operator+=(const fp2_t& b) {
    add_mod_384x(val, val, b.val, BLS12_377_P);
    return *this;
  }
  friend inline fp2_t operator+(const fp2_t& a, const fp2_t& b) {
    fp2_t ret;
  ...
  inline fp2_t& operator-=(const fp2_t& b) {
    sub_mod_384x(val, val, b.val, BLS12_377_P);
    return *this;
  }
  friend inline fp2_t operator-(const fp2_t& a, const fp2_t& b) {
    fp2_t ret;
  ...
  friend inline fp2_t operator*(const fp2_t& a, const fp2_t& b) {
    ...
    if (&a == &b) {
      ...
      sub_mod_384(
          ret.val[0], ret.val[0], ret.val[1], BLS12_377_P); // a0^2-5*a1^2
    } else {
      vec384 t0, t1;
