...
class __align__(((N + 63) / 64) & 1 ? 8 : 16) mont_t {
 public:
  ...
  static constexpr size_t __device__ bit_length() {
    return N;
  }
 ...
 private:
  ...
  static inline void cadd_n(
      uint32_t * acc, const uint32_t* a, size_t _n = (N + 31) / 32) {
  ...
  class wide_t {
   ...
   private:
    static inline void mad_row(
        uint32_t* odd,
        uint32_t* even,
        const uint32_t* a,
        uint32_t bi,
        size_t _n = (N + 31) / 32) {
      cmad_n(odd, a + 1, bi, _n - 2);
      asm("madc.lo.cc.u32 %0, %2, %3, 0; madc.hi.u32 %1, %2, %3, 0;"
          : "=r"(odd[_n - 2]), "=r"(odd[_n - 1])
          : "r"(a[_n - 1]), "r"(bi));

      cmad_n(even, a, bi, _n);
      asm("addc.u32 %0, %0, 0;" : "+r"(odd[_n - 1]));
    }
   ...
   public:
    inline wide_t(const mont_t& a, const mont_t& b) //// |a|*|b|
    {
      ...
      mul_n(odd, &a[1], b[0]);
      ++i;
      mad_row(&even[i + 1], &odd[i - 1], &a[0], b[i]);

#pragma unroll
      while (i < n - 2) {
        ++i;
        mad_row(&odd[i], &even[i], &a[0], b[i]);
        ++i;
        mad_row(&even[i + 1], &odd[i - 1], &a[0], b[i]);
      }
...
#pragma unroll
      while (i < n - 4) {
        ++i;
        mad_row(&even[2 * i + 2], &odd[2 * i], &a[i + 1], a[i], n - i - 1);
        ++i;
        qad_row(&odd[2 * i], &even[2 * i + 2], &a[i + 1], a[i], n - i);
      }
 ...
 private:
  static inline void madc_n_rshift(
...
#pragma unroll
 ...
 private:
  static inline void mul_by_1_row(
      uint32_t * even, uint32_t * odd, bool first = false) {
    uint32_t mi;
...
#pragma unroll
 ...
 public:
  static inline mont_t dot_product(
      const mont_t& a, const mont_t& b, const mont_t& c, const mont_t& d) {
    if (N % 32 == 0 || N % 32 > 30) {
...
#pragma unroll
  ...
  static inline mont_t dot_product(
      const mont_t a[], const mont_t* b, size_t len, size_t stride_b = 1) {
    size_t i;
    mont_t tmp;
    wide_t even;
...
#pragma unroll
 ...
 private:
  ...
  static inline void ab_approximation_n(
      approx_t & a_, const mont_t& a, approx_t& b_, const mont_t& b) {
    size_t i = n - 1;
...
#pragma unroll 2
    for (uint32_t n = 32 - 2; n--;) {
      ...
      asm("selp.b32          %0, %1, 0, !%odd;"
          "@%odd sub.cc.u32  %0, %1, %2;"
          : "=r"(a_b.lo)
          : "r"(a.lo), "r"(b.lo));
      asm("selp.b32          %0, %1, 0, !%odd;"
          "@%odd subc.cc.u32 %0, %1, %2;"
          : "=r"(a_b.hi)
          : "r"(a.hi), "r"(b.hi));
      asm("setp.gt.and.u32   %brw, %0, %1, %odd;" ::"r"(a_b.hi), "r"(a.hi));

      /* negate a_-b_ if it borrowed */
      asm("@%brw sub.cc.u32  %0, %1, %2;"
          : "+r"(a_b.lo)
          : "r"(b.lo), "r"(a.lo));
      asm("@%brw subc.cc.u32 %0, %1, %2;"
          : "+r"(a_b.hi)
          : "r"(b.hi), "r"(a.hi));
...
#pragma unroll 1
  ...
  static inline void smul_n_shift_x(
      mont_t & a, uint32_t & f_, mont_t & b, uint32_t & g_) {
    mont_t even, odd;
    uint32_t neg;
    size_t i;
  ...
  static inline uint32_t smul_2x(
      wide_t & u, uint32_t f, wide_t & v, uint32_t g) {
    wide_t even, odd;
    uint32_t neg;
