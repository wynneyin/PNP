...
from ..arithmetic import MSM,skip_leading_zeros_and_convert_to_bigints,convert_to_bigints,rand_poly,poly_add_poly_mul_const,evaluate,from_coeff_vec,poly_div_poly, \
                            skip_leading_zeros_and_convert_to_bigints_new,convert_to_bigints_new
...
class Commitment:
    ...
    @classmethod
    def commit_new(cls,powers,polynomial:torch.tensor,hiding_bound,params):
...
def commit_poly(ck:UniversalParams,polys,params):
    ...
    for labeled_poly in polys:
        ...
        comm,rand = Commitment.commit(powers,polynomial,hiding_bound,params)  ###ploynomial 为tensors
...
def commit_poly_new(ck:UniversalParams,polys,params):
    ...
    for labeled_poly in polys:
        ...
        comm,rand = Commitment.commit_new(powers,polynomial,hiding_bound,params)  ###ploynomial 为tensors
        labeled_comm.append(LabeledCommitment.new(label,comm))
        randomness.append(rand)
    return labeled_comm,randomness
