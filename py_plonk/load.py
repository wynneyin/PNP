...
import os
import torch
from .arithmetic import from_list_tensor,from_gmpy_list
def parse_bigint(s):
    start = s.find('"(') + 2
    end = s.find(')')
...
def read_pp_data_1(filename):
    ...
    h=AffinePointG2(x=G2Coordinate(c0=torch.tensor([1, 0, 0, 0],dtype=torch.BLS12_381_Fr_G1_Mont), c1=torch.tensor([2, 0, 0, 0],dtype=torch.BLS12_381_Fr_G1_Mont)), y=G2Coordinate(c0=torch.tensor([3, 0, 0, 0],dtype=torch.BLS12_381_Fr_G1_Mont), c1=torch.tensor([4, 0, 0, 0],dtype=torch.BLS12_381_Fr_G1_Mont)))
    beta_h=AffinePointG2(x=G2Coordinate(c0=torch.tensor([1, 0, 0, 0],dtype=torch.BLS12_381_Fr_G1_Mont), c1=torch.tensor([2, 0, 0, 0],dtype=torch.BLS12_381_Fr_G1_Mont)), y=G2Coordinate(c0=torch.tensor([3, 0, 0, 0],dtype=torch.BLS12_381_Fr_G1_Mont), c1=torch.tensor([4, 0, 0, 0],dtype=torch.BLS12_381_Fr_G1_Mont)))
...
def read_pk_data(filename):
    ...
    for name in ["q_m", "q_l", "q_r", "q_o", "q_4", "q_c", "q_hl", "q_hr", "q_h4", "q_arith"]:
        coeffs_key = f"data['arithmetic']['{name}']['coeffs']"
        evals_key = f"data['arithmetic']['{name}']['evals']"
        
        # 从 data 中获取 coeffs 和 evals
        coeffs = eval(coeffs_key)
        evals = eval(evals_key)
        ...
        if(len(evals)==0):
            ...
            from_gmpy_list(coeffs)
            from_gmpy_list(evals)
        # 应用两个函数
        q = (from_list_tensor(coeffs), from_list_tensor(evals))
        
        # 更新 data
        exec(f"{coeffs_key} = q[0]")
        exec(f"{evals_key} = q[1]")
    ...
    # 对 q_lookup 进行处理
    q_lookup_coeffs_key = "data['lookup']['q_lookup']['coeffs']"
    q_lookup_evals_key = "data['lookup']['q_lookup']['evals']"
    q_lookup_coeffs = eval(q_lookup_coeffs_key)
    q_lookup_evals = eval(q_lookup_evals_key)

    # 检查 coeffs 和 evals 是否为空列表
    if len(q_lookup_coeffs) == 0:
        q_lookup_coeffs = []
    if len(q_lookup_evals) == 0:
        q_lookup_evals = []
    else:
        from_gmpy_list(q_lookup_coeffs)
        from_gmpy_list(q_lookup_evals)

    q = (from_list_tensor(q_lookup_coeffs), from_list_tensor(q_lookup_evals))
    # 更新 data
    exec(f"{q_lookup_coeffs_key} = q[0]")
    exec(f"{q_lookup_evals_key} = q[1]")

    # 对 table_1 到 table_4 进行相同的处理
    for i in range(1, 5):
        table_coeffs_key = f"data['lookup']['table{i}']['coeffs']"
        table_coeffs = eval(table_coeffs_key)

        # 检查 coeffs 是否为空列表
        if len(table_coeffs) == 0:
            table_coeffs = []
        else:
            from_gmpy_list(table_coeffs)

        table_coeffs=from_list_tensor(table_coeffs)
        # 更新 data
        exec(f"{table_coeffs_key} = table_coeffs")
    ...
    # 定义要处理的数组和它们的键
    arrays_to_process = {
        "left_sigma": "data['permutation']['left_sigma']",
        "right_sigma": "data['permutation']['right_sigma']",
        "out_sigma": "data['permutation']['out_sigma']",
        "fourth_sigma": "data['permutation']['fourth_sigma']",
    }

    # 遍历要处理的数组
    for name, key in arrays_to_process.items():
        coeffs_key = f"{key}['coeffs']"
        evals_key = f"{key}['evals']" 
    ...
    from_gmpy_list(data["linear_evaluations"]["evals"])
    data["linear_evaluations"]["evals"]=from_list_tensor(data["linear_evaluations"]["evals"])
    ...
    # 定义要处理的数组和它们的键
    arrays_to_process = {
        "range_selector": "data['range_selector']",
        "logic_selector": "data['logic_selector']",
        "fixed_group_add_selector": "data['fixed_group_add_selector']",
        "variable_group_add_selector": "data['variable_group_add_selector']",
    }
    ...
    for name, key in arrays_to_process.items():
        ...
        # 从 data 中获取 coeffs 和 evals
        coeffs = eval(coeffs_key) if name != "v_h_coset_8n" else []
        evals = eval(evals_key)
        ...
        coeffs = from_list_tensor(coeffs)
        ...
        # 更新 data
        exec(f"{coeffs_key} = coeffs")
        exec(f"{evals_key} = evals")


    from_gmpy_list(data["v_h_coset_8n"]["evals"])
    data["v_h_coset_8n"]["evals"]=from_list_tensor(data["v_h_coset_8n"]["evals"])
...
def read_scalar_data_new(filename):
    import re
    ...
    for line in file_content:
        ...
        if match:
            ...
            list_data = [int(x) for x in list_data.split(',')]
    ...
    for i in range(len(list_of_lists)):
        # print(input[i].value)
        base_input.append(list_of_lists[i])
    output = torch.tensor(base_input,dtype = torch.BLS12_381_Fr_G1_Mont,device='cpu')
    return output