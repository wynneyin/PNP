...
static void params_zkp_template(Tensor& self, Tensor& local_params, bool is_intt){
    AT_DISPATCH_FR_MONT_TYPES(self.scalar_type(), "load_ntt_params_cuda", [&] {     
        ...
        auto local_ptr = reinterpret_cast<scalar_t::compute_type*>(local_params.mutable_data_ptr<scalar_t>());
        NTTParameters(is_intt, self_ptr, local_ptr);
    });
}

Tensor params_zkp_cuda(int64_t domain_size,  bool is_intt, 
    ...
    auto local_params = at::empty({double_group_gen + double_roots, num_uint64(*dtype)}, dtype, layout, device, pin_memory, c10::nullopt); 
...
static void ntt_zkp(Tensor& self, const Tensor& params, bool is_intt, bool is_coset) {
...
Tensor ntt_zkp_cuda(const Tensor& inout, const Tensor& params, bool is_intt, bool is_coset) {
    Tensor output = inout.clone();
    ntt_zkp(output, params, is_intt, is_coset);
    return output;
}

Tensor& ntt_zkp_cuda_(Tensor& inout, const Tensor& params, bool is_intt, bool is_coset) {
    ntt_zkp(inout, params, is_intt, is_coset);
    return inout;
}

Tensor& ntt_zkp_out_cuda(const Tensor& inout, const Tensor& params, bool is_intt, bool is_coset, Tensor& output) {
    copy(output, inout);   
    ntt_zkp(output, params, is_intt, is_coset);
    return output;
}
